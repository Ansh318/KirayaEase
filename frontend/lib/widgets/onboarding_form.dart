// // lib/widgets/onboarding_form.dart
...
//                         Uri.parse("http://127.0.0.1:8000/user-onboarding"), // Replace IP if needed
...
class _OnboardingFormState extends State<OnboardingForm> {
  ...
  Future<void> _submit() async {
    ...
    final payload = {
      ...
      "first_name": _firstNameController.text.trim(),
      "last_name": _lastNameController.text.trim(),
      "dob": _dobController.text.trim(), // already in YYYY-MM-DD
      "aadhaar": _aadharController.text.trim(),
      "pan": _panController.text.trim(),
      "role": _role?.toLowerCase(),
    };
    ...
    try {
      final response = await http.post(
        Uri.parse(
            "http://127.0.0.1:8000/user-onboarding"), // 🛠 Replace with your backend if needed
        headers: {"Content-Type": "application/json"},
        body: jsonEncode(payload),
      );
